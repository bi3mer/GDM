    ...
    for y in reversed(range(MAX_Y)):
        ...
        for x in range(MAX_X):
            if x == 1 and y == 1:
                ...
                out +=  '{:.3f} | '.format(G.reward(key) + calculate_max_utility(G, key, 0.9))
...
def __small_negative_reward_tester(rl_lambda):
    seed(0)
    start, G = __build_grid_world(-0.01)
    pi = rl_lambda(G, start)
    assert pi != None
...
def __medium_negative_reward_tester(rl_lambda):
    seed(0)
    start, G = __build_grid_world(-0.1)
    pi = rl_lambda(G, start)
    assert pi != None
...
def __very_negative_reward_tester(rl_lambda):
    seed(0)
    start, G = __build_grid_world(-1.5)
    pi = rl_lambda(G, start)
    assert pi != None
...
##### Value Iteration
value_iteration_lambda = lambda G, _: ADP.value_iteration(G, N, GAMMA, THETA)
...
in_place_value_iteration_lambda = lambda G, _: ADP.value_iteration(G, N, GAMMA, THETA, in_place=True)
...
##### Policy Iteration
policy_iteration_lambda = lambda G, _: ADP.policy_iteration(G, GAMMA)
...
in_place_policy_iteration_lambda = lambda G, _: ADP.policy_iteration(G, GAMMA, in_place=True)
...
modified_policy_iteration_lambda = lambda G, _: ADP.policy_iteration(G, GAMMA, modified=True)
...
modified_in_place_policy_iteration_lambda = lambda G, _: ADP.policy_iteration(G, GAMMA, in_place=True, modified=True)
